...
|:-:|-|-|-|
|CHAR[(N BYTE)]| 0 < N <= 255|CHAR[(N)]|Oracle的字符范围为：1 ~ floor(N/M) ；MySQL的字符范围为：0 ~ N|
|CHAR[(N BYTE)]| 255 < N <= 2000|VARCHAR(N)|MySQL的字符范围：0 ~ floor(N/M)，但是受单行64k大小限制，以及额外的1~2字节（取决于值是否大于255字节）|
