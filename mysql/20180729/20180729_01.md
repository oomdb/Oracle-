...
|NCHAR[(N)]|255 < N <= 1000|NVARCHAR(N)|Oracle字符范围：1 ~ floor(N/M)；MySQL的字符范围：0 ~ floor(N/M)，但是受单行64k大小限制，以及额外的1~2字节（取决于值是否大于255字节）|
|NVARCHAR2(N)\|NVARCHAR(N)|0 < N <= 2000|NVARCHAR(N)|Oracle字符范围：1 ~ floor(N/M)；MySQL的字符范围：0 ~ floor(N/M)，但是受单行64k大小限制，以及额外的1~2字节（取决于值是否大于255字节）|
