...
|Oracle|转换规则|MySQL|
|:-:|-|-|
|NUMBER\|NUMBER(\*)|1 <= p <= 38, 0 <= s <= 30 &nbsp;DECIMAL(p,s);</br>1 <= p <= 38,-84 <= s < 0 or 30 <= s <= 127 &nbsp;DOUBLE|待重新处理|
...
|NUMBER(p,s),p > s,s > 0|1 <= p <= 38, 0 <= s <= 30  &nbsp;&nbsp;DECIMAL(p,s);</br> 1 <= p <= 38, 30 <= s <= 127  &nbsp;&nbsp;VARCHAR(p)
|ECIMAL(p,s) 或者 VARCHAR(p)|
|NUMBER(p,s),p > s,s < 0|...|...|
|NUMBER(p,s),p < s|...|...|
|FLOAT|...|...|
|FLOAT(p)|...|...|
