...
|TIMESTAMP WITH TIME ZONE|Oracle默认fsp是6，MySQL默认fsp是0|...|
|TIMESTAMP(fsp) WITH TIME ZONE|6 < fsp <=9 不支持|...|
|TIMESTAMP WITH LOCAL TIME ZONE|Oracle默认fsp是6，MySQL默认fsp是0|...|
|TIMESTAMP(fsp) WITH LOCAL TIME ZONE|6 < fsp <=9 不支持|...|
|INTERVAL YEAR TO MONTH|...|不支持|
|INTERVAL YEAR (yp) TO MONTH|...|不支持|
|INTERVAL DAY [(dp)] TO SECOND [(fsp)]|...|不支持|
