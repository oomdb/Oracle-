...
|INTERVAL YEAR [(year_precision)] TO MONTH|占用大小是：固定 5字节|year_precision 是 YEAR的位数，可接受的值是 0 ~ 9，默认值是 2|...|
|INTERVAL DAY [(day_precision)] TO SECOND [(fractional_seconds_precision)]|占用大小：固定 11字节|day_precision 是 DAY的最大数值位，可接受的值是 0 ~ 9，默认值是 2;fractional_seconds_precision 是 SECOND 的位数，可接受的值是 0 ~ 9，默认值是 6|...|
